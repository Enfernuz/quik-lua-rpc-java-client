...
enum GetClassesListRequestPbSerde implements Serde<GetClassesList.Request> {

    INSTANCE;

    private static final RPC.ProcedureType PROCEDURE_TYPE = RPC.ProcedureType.GET_CLASSES_LIST;
    private static final byte[] PB_SERIALIZED_FORM = createPbSerializedForm();

    @Override
    public byte[] serialize(final GetClassesList.Request request) {
    ...
    @Override
    public GetClassesList.Request deserialize(final byte[] data) {

        final RPC.Request request = RPC.Request.parseFrom(data);
        assertEquals(request.getType(), PROCEDURE_TYPE);

        return GetClassesList.Request.INSTANCE;
    }
